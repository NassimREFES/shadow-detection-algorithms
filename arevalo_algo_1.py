...
class Seed:
    ...
    def __maj_uf(self):
        self.__uniq = np.array(self.hist.keys())
        self.__freq = [len(v) for v in np.array(self.hist.values())]
...
def selection_seeds(c3, c3_moy, v, s, noyau, ML):
    ...
    new_tv = sum(sum(normlized_v)) / (rows * cols)
    ...
    plt.hist(normlized_v.ravel(),256,[0,1])
    g_line = plt.axvline(new_tv, color='g', linestyle='-')
    r_line = plt.axvline(new_tv - 0.10, color='r', linestyle='-')
    plt.xlabel(u"pixel normalisé")
    plt.ylabel(u"fréquence")
    plt.title(u"Histogramme de V ( du HSV )")
    plt.legend(handles=[g_line, r_line], labels=["Moyenne de V", "Moyenne de V - 0.10"])
    ...
    return [regions, seeds, seeds_window, Tv]         
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        # fermeture_morph = cv2.morphologyEx(g_seeds, cv2.MORPH_CLOSE, elem_struct)
